...
public class TransformNode extends AbstractNode {
  ...
  public Matrix4f getObjectToViewTransform(Matrix4f accumulator, Matrix4f worldToView) {
    ...
    Matrix4f m2 = new Matrix4f()
            .mul(transform)
            .mul(animation_transform).mul(accumulator);
...
//    System.out.println("Testing intersection of transform node: " + name);
    currentTransformationStack.push(
            new Matrix4f(currentTransformationStack.peek())
              .mul(new Matrix4f(animation_transform))
              .mul(new Matrix4f(transform)));

//    // Add unvisited child to the list:
//    unvisited.add(child);
//
//    // Remove this node from the list of unvisited nodes:
//    unvisited.remove(this);

    // M view < animation * M animation < transform * M transform < object
    child.intersect(currentTransformationStack, viewRay, hit, textureImages);

    currentTransformationStack.pop();
  }
